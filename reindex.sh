...
mkdir -p "$(pwd)"/log
echo "$DB" > "$(pwd)"/log/reindex.log
date >> "$(pwd)"/log/reindex.log
echo "begin reindex" >> "$(pwd)"/log/reindex.log
for I in ${DB}; do
PGPASSWORD="$PASS" psql --dbname="$I" --host="$HOST" --port="$PORT" --username="$USER" -w --command "reindex DATABASE \"$I\";" 2>> "$(pwd)"/log/analyze.log
done
date >> "$(pwd)"/log/reindex.log
echo "end reindex" >> "$(pwd)"/log/reindex.log
mail -s 'Service "REINDEX PostgreSQL"' "$MAILTO" < "$(pwd)"/log/reindex.log
